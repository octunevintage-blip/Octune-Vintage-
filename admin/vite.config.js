...
export default defineConfig(({ mode }) => {
  // Load environment variables from the current directory (admin)
  const envAdmin = loadEnv(mode, __dirname, '');

  const processEnv = {};
  Object.keys(envAdmin).forEach((key) => {
    if (key.startsWith('NEXT_PUBLIC_') || key.startsWith('VITE_') || key === 'NODE_ENV') {
      processEnv[key] = envAdmin[key];
    }
  });
